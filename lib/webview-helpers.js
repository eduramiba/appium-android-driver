...
async function webviewsFromProcs (adb, deviceSocket) {
  ...
  for (let line of out.split("\n")) {
    ...
    let match = line.match(/@?webview_devtools_remote_(\d+)/);
    let webviewPid = null;
    if(match){
      webviewPid = match[1];
    } else {
    ...
    if(webviewPid){
      logger.debug(`line '${line}' mapped to webviewPid '${webviewPid}'`);
    }
...
async function procFromWebview (adb, webview) {
  if(webview.startsWith(WEBVIEW_BASE + XWALK_PREFIX)){
    return webview.split(WEBVIEW_BASE + XWALK_PREFIX)[1];
  }
